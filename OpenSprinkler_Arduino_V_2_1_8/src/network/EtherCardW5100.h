...
#ifndef _ETHERCARD_W5100_h
#define _ETHERCARD_W5100_h
...
private:
    ...
    static EthernetServer incoming_server;			// server used for incoming connections
    static EthernetClient incoming_client;			// client used for incoming www requests (only)
	static EthernetClient outgoing_client;			// client used for outgoing browse_url requests (only)
    static EthernetUDP udp_client;					// udp object used for NTP requests
...
public:
    ...
    */
    ...
    static void browseUrlRamHost(const char *urlbuf, const char *urlbuf_varpart,
        const char *hoststr, void(*callback)(uint8_t, uint16_t, uint16_t));
...
extern EtherCardW5100 etherW5100; //!< Global presentation of EtherCard class

#endif // _ETHERCARD_W5100_h
