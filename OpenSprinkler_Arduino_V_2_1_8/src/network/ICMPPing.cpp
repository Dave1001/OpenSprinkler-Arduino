...
uint16_t ICMPPing::ping_timeout = ICMP_PING_TIMEOUT;
...
void ICMPPing::receiveEchoReply(const ICMPEcho& echoReq, const IPAddress& addr, ICMPEchoReply& echoReply)
{
    ...
    while (millis() - start < ping_timeout)
    {
        ...
        uint8_t ipHeader[6];
		uint8_t _buffer = W5100.readSnRX_RD(_socket);
		W5100.read_data(_socket, (uint16_t) _buffer, ipHeader, sizeof(ipHeader));
		_buffer += sizeof(ipHeader);
		for (int i = 0; i < 4; ++i)
		...
		W5100.read_data(_socket, (uint16_t) _buffer, serialized, dataLen);
		echoReply.data.deserialize(serialized);

		_buffer += dataLen;
		W5100.writeSnRX_RD(_socket, _buffer);
