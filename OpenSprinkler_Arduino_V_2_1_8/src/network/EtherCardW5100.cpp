...
/// <summary>
/// Prepare HTTP post message - Request sent in main packetloop
/// </summary>
/// <param name="urlbuf">Pointer to c-string URL folder</param>
/// <param name="hoststr">hoststr Pointer to c-string hostname</param>
/// <param name="additionalheaderline">Pointer to c-string with additional HTTP header info</param>
/// <param name="postval">Pointer to c-string HTML Post value</param>
/// <param name="callback">Pointer to callback function to handle response</param>
void EtherCardW5100::httpPost(const char *urlbuf, const char *hoststr, const char *additionalheaderline, const char *postval, void(*callback)(uint8_t, uint16_t, uint16_t)) {
    ...
    // TODO TODO www_fd = clientTcpReq(&www_client_internal_result_cb, &www_client_internal_datafill_cb, hisport);
}
...
/// <param name="urlbuf">Pointer to c-string URL folder</param>
/// <param name="hoststr">hoststr Pointer to c-string hostname</param>
/// <param name="urlbuf_varpart">Pointer to c-string with additional URL info</param>
/// <param name="postval">Pointer to c-string HTML Post value</param>
/// <param name="callback">Pointer to callback function to handle response</param>
void EtherCardW5100::httpPostVar(const char *urlbuf, const char *hoststr, const char *urlbuf_varpart, const char *postval, void(*callback)(uint8_t, uint16_t, uint16_t)) {
    //client_urlbuf = urlbuf;
...
void EtherCardW5100::browseUrlRamHost(const char *urlbuf, const char *urlbuf_varpart, const char *hoststr, const char *additionalheaderline, void(*callback)(uint8_t, uint16_t, uint16_t))
{
    client_urlbuf = urlbuf;
    client_urlbuf_var = urlbuf_varpart;
    client_hoststr = hoststr;
    ...
    client_postval = 0;
    client_browser_cb = callback;
    // client_tcp_datafill_cb = &www_client_internal_datafill_cb_ramhost;
    ...
    // fill the buffer
    uint16_t len = (*www_client_internal_datafill_cb_ramhost) (www_fd);
    buffer[bfill.position() + TCP_OFFSET] = '\0';

    DEBUG_PRINT(F("Browse URL RAM Host: "));
    ...
    if (outgoing_client.connect(hoststr, hisport))
    {
        ...
        DEBUG_PRINT(F("Browse URL RAM Host: sent to "));
        DEBUG_PRINT(hoststr);
        DEBUG_PRINT(F(" port "));
        DEBUG_PRINT(hisport);
        DEBUG_PRINTLN(F("(OK)"));
