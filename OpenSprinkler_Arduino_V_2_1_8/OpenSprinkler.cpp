...
bool OpenSprinkler::read_hardware_mac() {
  ...
  return true;
#elif defined(OPENSPRINKLER_ARDUINO_W5100) || defined (OPENSPRINKLER_ARDUINO_ENC28J60)
  // always use software MAC address for Opensprinkler Arduino
  return false;
#else
  uint8_t ret;
...
void OpenSprinkler::begin() {
...
#elif defined (OPENSPRINKLER_ARDUINO_DISCRETE)
  OpenSprinklerDiscrete.begin();
#else // OPENSPRINKLER_ARDUINO_DISCRETE
  ...
  lcd_start();
    ...
    OpenSprinklerButton.begin();
#else // OPENSPRINKLER_ARDUINO_FREETRONICS_LCD 
  // set button pins
...
void OpenSprinkler::apply_all_station_bits() {
  ...
  byte bid, s, sbits;  
    ...
    // do the station processing seperately
    OpenSprinklerDiscrete.apply(status.enabled, station_bits, MAX_EXT_BOARDS);
    byte bid, s, sbits;
...
#if !defined(ARDUINO) || defined(__AVR_ATmega1284P__) || defined(__AVR_ATmega1284__) || defined(ESP8266) || defined(OPENSPRINKLER_ARDUINO)
...
void httpget_callback(byte status, uint16_t off, uint16_t len) {
  ...
  // OPENSPRINKLER_ARDUINO_W5100 OPENSPRINKLER_ARDUINO_ENC28J60
  // Ethernet::buffer[off+ETHER_BUFFER_SIZE-1] = 0;
  // DEBUG_PRINTLN((const char*) Ethernet::buffer + off);
  ether.buffer[off+ETHER_BUFFER_SIZE-1] = 0;
  DEBUG_PRINTLN((const char*) ether.buffer + off);
#endif
}
...
{
    return OpenSprinklerButton.read(waitmode);
}
#else
